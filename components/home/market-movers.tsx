      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-transparent shadow-[0_1px_2px_rgba(0,0,0,0.2)]">
        ...
        <div className="divide-y divide-white/10">
          {rows.map((card, index) => {
            ...
            return (
              <div key={card.id} className="transition-colors hover:bg-white/[0.02]">
                {/* Mobile View */}
                <div className="flex flex-col gap-2 px-4 py-4 md:hidden">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="font-medium text-zinc-200 truncate">{card.name}</div>
                    </div>
                    <div className="text-right font-medium text-white shrink-0">
                      {format(card.price)}
                    </div>
                  </div>
                  <div className="flex justify-between items-center pl-[52px]">
                    <div className="text-xs text-zinc-500 truncate mr-2">{card.set} &bull; {card.grade}</div>
                    <div className={cn('flex items-center gap-1 text-sm font-medium shrink-0', isPositive ? 'text-emerald-400' : 'text-rose-400')}>
                ...
                {/* Desktop View */}
                <div className="hidden md:grid md:grid-cols-[3rem_1.6fr_0.8fr_0.8fr_0.7fr_0.8fr_0.8fr] md:items-center md:gap-4 px-5 py-4">
