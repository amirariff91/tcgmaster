 ...
 */
async function verifyCard(card: any): Promise<boolean> {
  ...
  if (updated) {
    ...
    if (cacheNeedsRebuild && !isTestMode) {
      console.log(`  Rebuilding price_cache on the cards table to instantly remove corrupt prices...`);
      ...
      const newGradedPrices: Record<string, Record<string, number>> = {};
      const seenRaw = new Set();
      const seenGraded = new Set();

      if (recentHistory) {
        for (const h of recentHistory) {
          if (!h.grade || h.grade === 'raw') {
            if (!seenRaw.has(h.source)) {
              newRawPrices[h.source] = h.price;
              seenRaw.add(h.source);
            }
          } else {
            const key = `${h.grade}-${h.source}`;
            if (!seenGraded.has(key)) {
              if (!newGradedPrices[h.grade]) newGradedPrices[h.grade] = {};
              newGradedPrices[h.grade][h.source] = h.price;
              seenGraded.add(key);
            }
          }
        }
      ...
      const rawVals = Object.values(newRawPrices).filter(v => typeof v === 'number') as number[];
      if (rawVals.length > 0) {
        newRawPrices.market = Math.min(...rawVals);
      }

      await supabase.from('price_cache').upsert({
        card_id: card.id,
        raw_prices: newRawPrices,
        graded_prices: newGradedPrices,
