...
async function runSecScan() {
  console.log('🚀 Starting Secret Rare Alternate Art PriceCharting Set Scanner...');

  // Query all Secret Rare Alternate Art cards from our DB
  const dbSecs = await dbQuery<any>(`
    SELECT c.id, c.slug, c.name, c.number, c.pricecharting_url, s.name as set_name
    FROM cards c
    JOIN sets s ON c.set_id = s.id
    WHERE c.slug LIKE 'op-%-ja'
      AND s.name LIKE 'OP-%'
      AND (c.rarity ILIKE '%Secret%' OR c.rarity ILIKE '%SEC%' OR c.number LIKE '%-118_p1' OR c.number LIKE '%-119_p1' OR c.number LIKE '%-120_p1' OR c.number LIKE '%-121_p1' OR c.number LIKE '%-122_p1' OR c.number LIKE '%-123_p1')
  ...
  console.log(`📋 Found ${dbSecs.length} Secret Rare Alternate Art Japanese cards in database.`);
  ...
  try {
    for (const setInfo of SET_URLS) {
      ...
      $('table#games_table tbody tr').each((_, el) => {
        ...
        // Must indicate Alternate Art, but NOT Manga, NOT Serial, NOT PRB01 reprint
        ...
        // Match with DB SEC card
        const matched = dbSecs.filter(c => {
    ...
    console.log(`\n==============================================`);
    console.log(`🎉 Total Unique Secret Rare Alternate Arts Matched: ${discoveredMappings.length}`);
    ...
    for (const item of discoveredMappings) {
      ...
      await dbQuery(`
        INSERT INTO card_source_mapping (
          ...
          '{"origin": "sec-batch-scan"}'::jsonb, NOW(), NOW()
        )
    ...
    console.log(`✅ Successfully locked and confirmed ${lockedCount} Secret Rare Alternate Art cards in database!`);
  } finally {
    await browser.close();
  }
}

runSecScan().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
