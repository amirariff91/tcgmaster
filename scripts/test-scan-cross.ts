...
async function testScan() {
  ...
  try {
    ...
    rows.each((_, el) => {
      ...
      if (lower.includes('op01-') || lower.includes('op02-') || lower.includes('op03-') || lower.includes('op04-') || lower.includes('st01-')) {
        console.log(`Cross-set match: "${title}" | Raw: ${price} | https://www.pricecharting.com${href}`);
      }
    });
  } finally {
    await browser.close();
