...
async function run() {
  ...
  while (true) {
    ...
    // 1. TCGPlayer (Fast)
    console.log('[English DBFW] Fetching from TCGPlayer...');
    ...
    console.log('[English DBFW] Fetching from PriceCharting...');
    const pcResult = await fetchPriceChartingPrice(card.number);
    if (pcResult !== null) {
      results.push({ price: pcResult.price, source: 'pricecharting', grade: 'raw' });
      console.log(`[English DBFW] PriceCharting: $${pcResult.price}`);
      if (pcResult.gradedPrice) {
        results.push({ price: pcResult.gradedPrice, source: 'pricecharting', grade: 'psa10' });
        console.log(`[English DBFW] PriceCharting PSA 10: $${pcResult.gradedPrice}`);
      }
    }
    
    if (results.length > 0) {
      console.log(`[English DBFW] Successfully fetched ${results.length} price points.`);
    ...
    console.log(`[English DBFW] Sleeping for ${SLEEP_MS / 1000}s... Zzz...\n`);
