...
async function run() {
  ...
  for (const card of cards) {
    ...
    if (card.snkrdunk_url) {
      mappings.push({ source: 'snkrdunk', externalUrl: card.snkrdunk_url, confidence: 'confirmed' });
    }
    if (card.yuyutei_url) {
      mappings.push({ source: 'yuyutei', externalUrl: card.yuyutei_url, confidence: 'confirmed' });
    }
    if (card.pricecharting_url) {
      mappings.push({ source: 'pricecharting', externalUrl: card.pricecharting_url, confidence: 'confirmed' });
    }
    ...
    try {
      const result = await fetchCard(card as any, mappings);
