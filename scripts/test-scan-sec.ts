...
async function scanOp01Sec() {
  ...
  try {
    ...
    rows.each((_, el) => {
      ...
      if (title.includes('OP01-120') || title.includes('OP01-121')) {
        console.log(`SEC Match: "${title}" | Raw: ${price} | https://www.pricecharting.com${href}`);
      }
    });
  } finally {
    await browser.close();
...
scanOp01Sec().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
